...
- **공개 계정이어야 한다.** 비공개 계정은 이 방식으로 연결되지 않는다.
- 페이스북 계정 (Meta 개발자 콘솔 로그인에 필요하다. 페이지는 없어도 된다)
- 파이썬 3.9 이상
...
왼쪽 메뉴의 **Instagram → API setup with Instagram business login** 으로 들어간다.
(한국어 콘솔이면 「Instagram 비즈니스 로그인을 사용한 API 설정」)

**예전에 필요했던 '테스터 추가' 절차는 이제 없다.** 내 계정을 바로 붙이면 된다.

1. **1단계 · Instagram 계정 추가**(Add an Instagram account) 를 누른다
2. 인스타 계정으로 로그인하고 권한을 허용한다
3. 붙은 계정 옆에 요청되는 권한에 아래가 들어 있는지 본다
   - `instagram_business_basic`
   - `instagram_business_manage_insights`  ← **이게 없으면 지표를 못 받는다**
   - (댓글까지 읽고 싶다면 `instagram_business_manage_comments`)

## 3. 액세스 토큰 받기

같은 화면에서 붙인 계정 옆의 **토큰 생성**(Generate token) 을 누른다. 인스타 로그인 창이 뜨고,
허용하면 토큰 문자열이 나온다.

- **바로 복사해 둬라.** 이 창을 닫으면 같은 토큰을 다시 볼 수 없다. (다시 생성은 된다)
- 이 화면에서 나오는 토큰은 **처음부터 장기 토큰(60일)** 이다. 따로 교환할 필요가 없다.
- 60일이 지나기 전에 갱신해야 한다. 아래 5번의 `--refresh-token`이 그 일을 한다.

> 토큰은 비밀번호와 같다. 캡처해서 공유하거나 채팅창에 붙여넣지 마라.
> 실수로 노출했다면 같은 화면에서 다시 생성하면 이전 것이 무효가 된다.

> 화면 구성이 이 설명과 다르다면 Meta 가 콘솔을 바꾼 것이다.
> <https://developers.facebook.com/tools/explorer> (그래프 API 탐색기)에서
> 내 앱을 고르고 위 스코프를 체크해 단기 토큰을 받은 뒤,
> `python3 collect.py --exchange-token <단기토큰>` 으로 장기 토큰으로 바꿔도 된다.
